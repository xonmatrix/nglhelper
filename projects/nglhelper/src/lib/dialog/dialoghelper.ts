...
export class DialogHelper {
  ...
  AlertError(err: any, title: string = "Error") {
    ...
    let detailed = null;
    /*
        if (err instanceof HttpErrorResponse && !(err.error instanceof Error) && err.status === 500 && err.headers.get('content-type').indexOf('text/html') >= 0) {
            detailed = err.error;
        }*/
    return this.ShowMessageDialog({
      title: title,
    ...
    return this.dialog
      .open(PopUpDialogComponent, {
        ...
        data: {
          ...
          title: title,
          message: message,
          buttons: [{ Id: 1, Name: "Yes" }, { Id: 0, Name: "No" }]
        }
      })
      .afterClosed()
      .toPromise();
